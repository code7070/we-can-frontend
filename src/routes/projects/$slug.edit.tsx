...
function EditProjectForm() {
  ...
  const [groups, setGroups] = useState(() =>
    project.groups.length > 0
      ? project.groups.map((g, i) => ({ id: i + 1, value: g.title }))
      : [{ id: 1, value: "" }]
  );
  const [selectedMembers, setSelectedMembers] = useState<string[]>([]);
  const [nameError, setNameError] = useState("");
  const nextId = useRef(project.groups.length + 1);
  ...
  return (
    <div className="max-w-[720px] mx-auto px-8 pt-8 pb-20">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-text-secondary mt-1">Update project details and settings.</p>
      </div>
      ...
      <div className="flex flex-col gap-7">
        ...
        <div className="bg-surface border border-border rounded-xl p-6">
          ...
          <p className="text-sm text-text-disabled mb-4 -mt-2">
            Organize tasks into groups. Removing a group does not delete its tasks.
          </p>
